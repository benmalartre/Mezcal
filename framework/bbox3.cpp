...
namespace BOB{
void
BBox3::setMatrices(const Matrix4 &matrix)
{
    const float PRECISION_LIMIT = 1.0e-13;
    float det;
    ...
    if (fabs(det) <= PRECISION_LIMIT) {
        _isDegenerate = true;
        _inverse.identity();
    }
}

float BBox3::getVolume() const
{
    if (_box.IsEmpty())
        return 0.0;
