...
static inline float hash2vec1( int n, float x )
{
    n = (n << 13) ^ n;
...
static inline float hash2vec2( int n, float x, float y )
{
    n = (n << 13) ^ n;
    n = (n * (n * n * 15731 + 789221) + 1376312589);
...
static inline float hash2vec3( int n, float x, float y, float z )
{
    n = (n << 13) ^ n;
    n = (n * (n * n * 15731 + 789221) + 1376312589);
...
static inline int f2i( float x ) { return (int)floorf(x); }

static inline float smoothCub_f( float x ) { return x*x*(3.0f - 2.0f*x); }
static inline float smoothQui_f( float x ) { return x*x*x*(10.0f+x*(6.0f*x-15.0f)); }

static inline float smoothQuiDer_f( float x ) { return 30.0f*x*x*(x*(x-2.0f)+1.0f); }


static const unsigned char noise_permutation[] = {
...
float noise_1f_grad_lut( const float x )
{
    ...
    const float a = hash2vec1( noise_permutation[ ix0 ], fx0 );
    const float b = hash2vec1( noise_permutation[ ix1 ], fx1 );
...
float noise_1f_grad_hash( const Vector2  & x, const int pe )
{
    ...
    int A = noise_permutation[(i[0]  )&pe]+(i[1])&pe, AA = noise_permutation[A], AB = noise_permutation[A+1];
    int B = noise_permutation[(i[0]+1)&pe]+(i[1])&pe, BA = noise_permutation[B], BB = noise_permutation[B+1];
...
*/
Vector3 noise_3f_grad_hash(const float p)
{
...
/*
Vector2 noise_2f_grad_hash(const Vector3 & x)
{
    const int   i[3] = { f2i(x[0]), f2i(x[1]), f2i(x[2]) };
    const float u[3] = { x[0] - (float)i[0],   x[1] - (float)i[1],   x[2] - (float)i[2] };
    const float w[3] = { smoothQui_f(u[0]),      smoothQui_f(u[1]),      smoothQui_f(u[2]) };
...
float noise_1f_grad_hash( const Vector3 & x )
{
    ...
    int A = noise_permutation[(i[0]  )&255]+i[1], AA = noise_permutation[(A)&255]+i[2], AB = noise_permutation[(A+1)&255]+i[2];
    int B = noise_permutation[(i[0]+1)&255]+i[1], BA = noise_permutation[(B)&255]+i[2], BB = noise_permutation[(B+1)&255]+i[2];
...
*/
Vector3 noise_1f_3f_value_hash( const float x )
{
	const int   i = { f2i(x) };
	const float u = { x-(float)i };
    const float w = { smoothQui_f(u) };
...
/*
Vector2 noise_2f_value_hash( const Vector2 & x )
{
	const int   i[2] = { f2i(x[0]), f2i(x[1]) };
	const float u[2] = { x[0]-(float)i[0],   x[1]-(float)i[1] };
    const float w[2] = { smoothQui_f(u[0]),      smoothQui_f(u[1]) };
...
Vector2 noise_2f_value_hash(const Vector3 & x)
{
    const int   i[3] = { f2i(x[0]), f2i(x[1]), f2i(x[2]) };
    const float u[3] = { x[0] - (float)i[0],   x[1] - (float)i[1],   x[2] - (float)i[2] };
    const float w[3] = { smoothQui_f(u[0]),      smoothQui_f(u[1]),      smoothQui_f(u[2]) };
...
*/
Vector3 noise_3f_value_hash(const float p)
{
...
float noise_1f_value_hash( const Vector3 & x )
{
    ...
    /*
Vector2 noise_3f_2f_value_hash( const Vector3 & x )
{
	const int   i[3] = { f2i(x[0]), f2i(x[1]), f2i(x[2]) };
	const float u[3] = { x[0]-(float)i[0],   x[1]-(float)i[1],   x[2]-(float)i[2] };
    const float w[3] = { smoothQui_f(u[0]),      smoothQui_f(u[1]),      smoothQui_f(u[2]) };
